...
//import { getInitialBoard } from '../gameData/tangoData';
//import { initialBoards } from '../gameData/tangoBoards';
//import { gameExclusiveData } from '../gameData/sudokuData';
...
const Board=()=>{
  ...
  useEffect(()=>{
    ...
    // State for gameExclusiveData to hold updated data
    const [gameExclusiveData, setGameExclusiveData] = useState(currentGame.gameExclusiveData);


  //board Switching
  const handleBoardSwitch = () => {
    console.log('from boardPage: ',typeof(currentGame.getInitialBoard(boardIndex)),' \n', typeof(gameExclusiveData.initialBoard));
    console.log('below ', currentGame.getInitialBoard(boardIndex));
    const updatedGame = currentGame.boardSwitcher(gameExclusiveData.initialBoard, gameExclusiveData);
...
//renders the board updates immediately where in setBoard(updatedBoard) only schedules the changes until enxt render
  useEffect(() => {
  ...
  useEffect(()=>{
    ...
    //   currentGame.gameRules(getInitialBoard()); // Apply new game rules
    // }
    console.log('Data from use effect for gameExclusiveData and currentGame, \n ',gameExclusiveData)
  },[gameExclusiveData,currentGame]);
  ...
  const handleCellClick=(rowIdx:number, colIdx:number)=>{
    ...
    if(gameExclusiveData.initialBoard[rowIdx][colIdx]==='_'){
            ...
            {board.map((row, rowIdx) => (
              <div key={rowIdx} style={{ display: 'flex' }}>
                {row.map((cell, colIdx) => {
                  ...
                  const immutableCell = currentGame.gameExclusiveData.initialBoard[rowIdx][colIdx]!=='_'; //check if the cell is mutable or not
